    ...
    <AdminLayout>
      <div className="max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-2 pt-2">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-foreground tracking-tight">Pengurusan & Analytics API Integrator</h1>
            </div>
            <p className="text-sm text-muted-foreground/90">Kawal selia Kunci API awam, pemantauan log permintaan & integrasi automatik</p>
          </div>

          <div className="flex items-center gap-3 w-full lg:w-auto">
            <button
              onClick={() => { setGeneratedKey(null); setShowGenerateModal(true); }}
              className="flex items-center gap-2 h-10 px-4 rounded-xl bg-gradient-to-r from-sky-500 to-indigo-600 hover:from-sky-600 hover:to-indigo-700 text-foreground font-bold text-xs transition-all shadow-md shadow-sky-500/10"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-[#14192B] border border-border/70 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground/90 mb-1">Jumlah Key API</p>
              <h3 className="text-2xl font-bold text-foreground">{stats?.totalKeys || 0}</h3>
          ...
          <div className="p-5 rounded-2xl bg-[#14192B] border border-border/70 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground/90 mb-1">Pesanan via API</p>
              <h3 className="text-2xl font-bold text-foreground">{stats?.totalApiOrders || 0}</h3>
              <p className="text-[11px] text-muted-foreground mt-1">Transaksi Integrasi Automatik</p>
            </div>
            <div className="h-10 w-10 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center text-primary">
          ...
          <div className="p-5 rounded-2xl bg-[#14192B] border border-border/70 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground/90 mb-1">Jualan via API (MYR)</p>
              <h3 className="text-2xl font-bold text-emerald-400">
                RM {Number(stats?.totalApiRevenue || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </h3>
              <p className="text-[11px] text-muted-foreground mt-1">Nisbah Volume API</p>
            </div>
          ...
          <div className="p-5 rounded-2xl bg-[#14192B] border border-border/70 flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-muted-foreground/90 mb-1">Konfigurasi Rate Limit</p>
              <h3 className="text-sm font-bold text-foreground mb-0.5">Read: {stats?.readRateLimit || "60/min"}</h3>
              <h3 className="text-sm font-bold text-amber-400">Order: {stats?.orderRateLimit || "10/min"}</h3>
            </div>
            <div className="h-10 w-10 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center text-amber-400">
        ...
        <div className="rounded-2xl bg-[#14192B] border border-border/70 p-5">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-foreground">Senarai Kunci API Integrator</h3>
            </div>
            <span className="text-xs text-muted-foreground">{keys.length} Kunci Terdaftar</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/5">
                {keys.length > 0 ? (
                  keys.map((k: any) => (
                    <tr key={k.id} className="hover:bg-secondary/50 transition-colors group">
                      <td className="py-3 px-3 font-mono text-xs font-bold text-sky-400">
                        {k.key_prefix}â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢
                      </td>
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-col gap-0.5 text-[10px]">
                          ...
                          {!k.user?.email && !k.user?.telegram_id && (
                            <span className="text-muted-foreground font-mono">ID: {k.user_id}</span>
                          )}
                        </div>
                      </td>
                      <td className="py-3 px-3 text-center">
                        {k.is_active ? (
                          ...
                          <span className="px-2 py-0.5 rounded-md bg-red-50 border border-red-200 text-red-600 text-[10px] font-bold inline-flex items-center gap-1">
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-red-50 border border-red-200 text-red-600 hover:bg-red-500/20 transition-all"
                          >
        ...
        <div className="rounded-2xl bg-[#14192B] border border-border/70 p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-foreground">Log Permintaan & Trafik API</h3>
            </div>
            <span className="text-xs text-muted-foreground">Sejarah Panggilan API Realtime</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/5">
                {apiLogs.length > 0 ? (
                  apiLogs.map((log: any) => (
                    <tr key={log.id} className="hover:bg-secondary/50 transition-colors text-xs">
                      <td className="py-3 px-3">
                        <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-bold text-[10px] mr-2">
                          {log.method}
                        </span>
                        <code className="text-foreground/85 font-mono">{log.endpoint}</code>
                      </td>
                      <td className="py-3 px-3 text-center">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${log.statusCode === 200 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-600'}`}>
                          {log.statusText}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-3 text-foreground/75">
                        {log.userIdentifier}
                      </td>
          ...
          {logsPagination && logsPagination.totalPages > 1 && (
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/70 text-xs text-muted-foreground/90">
              <span>Halaman {logsPage} daripada {logsPagination.totalPages}</span>
              <div className="flex items-center gap-2">
                <button
                  disabled={logsPage <= 1}
                  onClick={() => setLogsPage(logsPage - 1)}
                  className="p-1.5 rounded-lg bg-secondary/60 hover:bg-secondary/80 disabled:opacity-30"
                >
        ...
        <div className="rounded-2xl bg-[#14192B] border border-border/70 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-bold text-foreground">Status & Dokumentasi Endpoints API V1</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="p-4 rounded-xl bg-secondary/50 border border-border/70 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-muted-foreground">{ep.limit}</span>
                </div>
                <p className="font-mono text-xs font-bold text-foreground">{ep.path}</p>
                <p className="text-[11px] text-muted-foreground/90 leading-relaxed">{ep.desc}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <DialogContent className="bg-[#14192B] border border-border/80 text-foreground max-w-md">
            ...
            {generatedKey ? (
              <div className="space-y-4 pt-2">
                ...
                <div className="relative">
                  <input
                    ...
                    readOnly
                    value={generatedKey}
                    className="w-full h-11 pl-3 pr-12 rounded-xl bg-secondary border border-border/80 font-mono text-xs text-sky-700 select-all"
                  />
                  <button
                    type="button"
                    onClick={() => copyToClipboard(generatedKey)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg bg-sky-500 hover:bg-sky-600 text-foreground transition-all"
                  >
                ...
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-secondary/80 hover:bg-secondary text-xs text-foreground font-bold"
                  >
                    Tutup
                  </button>
                </div>
              ...
              <form onSubmit={handleGenerate} className="space-y-4 pt-2">
                <div>
                  <label className="text-xs font-semibold text-foreground/75 block mb-1">Pilih Pengguna Developer</label>
                  <select
                    value={selectedUserId}
                    onChange={(e) => setSelectedUserId(e.target.value)}
                    className="w-full h-10 px-3 rounded-xl bg-secondary/60 border border-border/80 text-sm text-foreground focus:outline-none focus:border-sky-400"
                  >
                ...
                <div className="p-3 rounded-xl bg-secondary/60 border border-border/70 text-[11px] text-muted-foreground/90">
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-border/70">
                  <button
                    type="button"
                    onClick={() => setShowGenerateModal(false)}
                    className="px-4 py-2 rounded-xl bg-secondary/60 hover:bg-secondary/80 text-xs text-foreground/75 font-semibold"
                  >
                    Batal
                  </button>
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-600 text-xs text-foreground font-bold transition-all"
                  >
