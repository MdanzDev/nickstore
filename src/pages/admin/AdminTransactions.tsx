...
export default function AdminTransactions() {
  ...
  const getStatusConfig = (status: string) => {
    ...
    if (["failed", "gagal", "cancelled", "expired"].includes(s))
      return { label: "Gagal/Batal", bg: "bg-red-500/10", text: "text-red-600", border: "border-red-500/30" };
    ...
    <AdminLayout>
      <div className="max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-2 pt-2">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-foreground tracking-tight">Pengurusan Transaksi</h1>
            </div>
            <p className="text-sm text-muted-foreground/90">Sejarah lengkap pergerakan baki, deposit, dan semua medan transaksi</p>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-[#14192B] border border-border/70 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-4 rounded-xl bg-secondary/60 border border-border/80 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-[#38BDF8]"
            />
          </div>

          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto hide-scrollbar">
            <span className="text-xs text-muted-foreground flex items-center gap-1 shrink-0 mr-1">
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-medium whitespace-nowrap transition-all ${
                  typeFilter === f.id
                    ? "bg-[#38BDF8] text-foreground font-bold"
                    : "bg-secondary/60 text-foreground/70 hover:text-foreground hover:bg-secondary/80"
                }`}
              >
                {f.label}
              </button>
        ...
        <div className="rounded-2xl bg-[#14192B] border border-border/70 p-5">
          {isLoading ? (
            <div className="py-16 text-center text-muted-foreground">Memuatkan rekod...</div>
          ) : transactions.length === 0 ? (
            <div className="py-16 text-center text-muted-foreground">Tiada transaksi</div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="space-y-1.5 w-full min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {providerStatus && (
                            <span className="px-2 py-0.5 rounded-md text-[10px] bg-secondary/60 text-foreground/70 border border-border/80">
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground/90">
                          <span className="flex items-center gap-1">
                            <User className="h-3 w-3 text-muted-foreground/60" /> {username}
                          </span>
                          {gameSlug && (
                            <span className="flex items-center gap-1 bg-secondary/60 px-1.5 py-0.5 rounded">
                              <Globe className="h-3 w-3 text-muted-foreground/60" />
                              <code className="text-primary">{gameSlug}</code>
                            </span>
                          )}
                          {zoneId && (
                            <span className="flex items-center gap-1 bg-secondary/60 px-1.5 py-0.5 rounded">
                    ...
                    <div className="flex flex-row lg:flex-col items-center lg:items-end justify-between gap-3 border-t border-border/70 lg:border-t-0 pt-3 lg:pt-0 min-w-[180px]">
                      <div className="text-right">
                        <p className={`font-bold text-base ${isDeposit ? "text-emerald-400" : "text-foreground"}`}>
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg bg-secondary/60 hover:bg-secondary/80 text-sky-400 transition-colors"
                      >
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-border/70 text-xs text-muted-foreground/90">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 rounded-lg bg-secondary/60 hover:bg-secondary/80 disabled:opacity-30"
                >
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <span className="font-bold text-foreground px-2">Halaman {page} daripada {totalPages}</span>
                <button
                ...
                <div className="p-4 rounded-xl bg-secondary/50 border border-border/70 space-y-2 text-xs">
                  {[
                    ...
                    <div key={item.label} className="flex justify-between border-b border-border/70 pb-1.5 last:border-0">
                      ...
                      <span className={`font-bold text-right max-w-[60%] break-all ${item.mono ? "font-mono" : ""} ${item.color || "text-foreground"}`}>
                        {item.value}
                      </span>
                    </div>
                  ))}
            ...
            <form onSubmit={handleAdjustBalance} className="space-y-4 pt-2">
              <div>
                ...
                <select
                  ...
                  className="w-full h-10 px-3 rounded-xl bg-secondary/60 border border-border/80 text-sm text-foreground focus:outline-none focus:border-emerald-400"
                >
              ...
              <div>
                <label className="text-xs font-semibold text-foreground/75 block mb-1">Catatan / Keterangan</label>
                <input
                  type="text"
                  value={adjustNote}
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-border/70">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-xs text-foreground font-bold transition-all"
                >
