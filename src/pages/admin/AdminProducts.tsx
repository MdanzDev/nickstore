...
const renderGameIcon = (iconUrl?: string) => {
  if (!iconUrl) return <Gamepad2 className="h-6 w-6 text-muted-foreground/40" />;
    ...
    <AdminLayout>
      <div className="max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-2">
          <div>
            <h1 className="text-2xl font-bold text-foreground tracking-tight">Katalog Produk & Game</h1>
            <p className="text-sm text-muted-foreground/90 mt-1">Pengurusan senarai game, ikon CDN Supabase & penetapan denominasi harga</p>
          </div>
          ...
          <div className="flex items-center gap-2 p-1 rounded-2xl bg-[#14192B] border border-border/80 self-start md:self-auto">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                activeTab === "products"
                  ? "bg-[#34D399] text-white shadow-md shadow-emerald-500/20"
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                activeTab === "games"
                  ? "bg-pink-500 text-foreground shadow-md shadow-pink-500/20"
                  : "text-foreground/70 hover:text-foreground hover:bg-secondary/60"
              }`}
            >
        ...
        {activeTab === "products" && (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full h-10 pl-10 pr-10 rounded-xl bg-[#14192B] border border-border/80 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-[#34D399]"
                />
                {search && (
              ...
              <button
                ...
                className="flex items-center gap-2 h-10 px-4 rounded-xl bg-[#14192B] border border-border/80 text-sm text-foreground/75 hover:text-foreground hover:border-border transition-all"
              >
                <RefreshCw className={`h-4 w-4 text-muted-foreground ${isLoadingProducts ? 'animate-spin' : ''}`} />
                Muat Semula
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {products.map((p: any) => (
                <div key={p.id} className="p-4 rounded-2xl bg-[#14192B] border border-border/70 space-y-3 flex flex-col justify-between hover:border-border/80 transition-all">
                  <div className="flex items-start gap-3">
                    <div className="w-12 h-12 rounded-xl bg-secondary/60 border border-border/80 overflow-hidden flex items-center justify-center shrink-0">
                      {renderGameIcon(p.icon || (p.images && p.images[0]))}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-bold text-sm text-foreground truncate">{p.name}</h3>
                      <p className="text-xs text-muted-foreground capitalize">{p.category || "Game Topup"}</p>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full h-10 pl-10 pr-10 rounded-xl bg-[#14192B] border border-border/80 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-pink-500"
                />
              </div>
              <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {games.map((g: any) => {
                ...
                return (
                  <div key={g.id || g.slug} className="p-4 rounded-2xl bg-[#14192B] border border-border/70 flex flex-col items-center text-center gap-3">
                    <div className="w-16 h-16 rounded-2xl bg-secondary/60 border border-border/80 overflow-hidden flex items-center justify-center shrink-0 relative">
                      {renderGameIcon(icon)}
                    </div>
                    <div className="w-full truncate">
                      <h3 className="font-bold text-xs text-foreground truncate">{g.name}</h3>
                      <p className="text-[10px] text-muted-foreground/60 truncate">{slug}</p>
                    </div>
        ...
        <Dialog open={!!selectedGameForDenoms} onOpenChange={() => setSelectedGameForDenoms(null)}>
          <DialogContent className="bg-[#14192B] border border-border/80 text-foreground max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-foreground font-bold text-lg">
                <Layers className="h-5 w-5 text-[#34D399]" /> Urus Denominasi & Harga â€” {selectedGameForDenoms?.name}
              </DialogTitle>
            </DialogHeader>
            ...
            {selectedGameForDenoms && (
              <div className="space-y-6 pt-2">
                ...
                <form onSubmit={handleAddDenom} className="p-4 rounded-xl bg-secondary/50 border border-border/70 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input
                      ...
                      className="h-10 px-3 rounded-xl bg-secondary/60 border border-border/80 text-xs text-foreground focus:outline-none focus:border-[#34D399]"
                    />
                    <div className="flex gap-2">
                      <input
                        ...
                        className="h-10 px-3 rounded-xl bg-secondary/60 border border-border/80 text-xs text-foreground font-mono focus:outline-none focus:border-[#34D399] w-full"
                      />
                      <button
                        type="submit"
                        disabled={createDenomMutation.isPending}
                        className="px-4 h-10 rounded-xl bg-[#34D399] text-white font-bold text-xs shrink-0 hover:bg-emerald-400 transition-all"
                      >
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-muted-foreground/90 uppercase tracking-wider">
                  ...
                  {isLoadingDenoms ? (
                    ...
                    <div className="space-y-2">
                      {denoms.map((d: any) => {
                        ...
                        return (
                          <div key={d.id} className="p-3 rounded-xl bg-secondary/50 border border-border/70 flex items-center justify-between gap-3 text-xs">
                            ...
                            {isEditing ? (
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className="h-8 w-24 px-2 rounded-lg bg-secondary border border-emerald-500 font-mono text-xs text-emerald-700"
                                />
                                <button
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-secondary/80 text-foreground/75"
                                >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-secondary/60 hover:bg-secondary/80 text-sky-400"
                                  title="Edit Harga"
                                >
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-600"
